...
class SendViewController: FormViewController {
    ...
    init(
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(getPassScanAddress(_:)), name: Notification.Names.QRCodeReadPassAddress, object: nil)
        
    }
    @objc func getPassScanAddress(_ noti: Notification) {
        if let address = noti.userInfo?["result"] as? String {addressRow?.value = address;addressRow?.reload()}
    }
    deinit {
        NotificationCenter.default.removeObserver(self, name: Notification.Names.QRCodeReadPassAddress, object: nil)
    }
