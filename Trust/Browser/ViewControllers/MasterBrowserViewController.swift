...
final class MasterBrowserViewController: UIViewController {
    ...
    private func updateView() {
        ...
        } else if segmentController.selectedSegmentIndex == BookmarksViewType.browser.rawValue {
            remove(asChildViewController: bookmarksViewController)
            remove(asChildViewController: historyViewController)
    ...
    @objc func selectionDidChange(_ sender: UISegmentedControl) {
        ...
        guard let reviewType = BookmarksViewType(rawValue: 3) else {
            return
        }
        //修改第一个进入主页   调整emun
        if sender.selectedSegmentIndex == 0 {
            delegate?.didPressAction(.view(reviewType))
        } else {
...
extension MasterBrowserViewController: Scrollable {
    func scrollOnTop() {
        //        browserViewController.goChangeHome()
    }
}
