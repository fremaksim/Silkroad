...
class InCoordinator: Coordinator {
    ...
    init(
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(qrReadAddress(_:)), name: Notification.Names.QRCodeReadAddress, object: nil)
    }
    @objc func qrReadAddress(_ noti: Notification) {
        if let address = noti.userInfo?["result"]  as? String{
            SRLog(message: address)
            // 找ETH钱包
            let searchTokensCoordinators  =  self.coordinators.compactMap{$0 as? TokensCoordinator}
            //TODO: 优化
            if let first = searchTokensCoordinators.first {
                let session = first.session
                let tokensDataStore = TokensDataStore(realm: session.realm, account: session.account)
                let objects = tokensDataStore.enabledObject
                var target: TokenObject?  = nil
                for obj in objects {
                    if obj.type == .coin {
                        target = obj
                        break
                ...
                if let target = target {
                    sendFlow(for: target)
                    NotificationCenter.default.post(name: Notification.Names.QRCodeReadPassAddress, object: nil, userInfo: ["result": address])
                }
            }
        }
    }
    ...
    deinit {
        NotificationCenter.default.removeObserver(self, name: Notification.Names.QRCodeReadAddress, object: nil)
