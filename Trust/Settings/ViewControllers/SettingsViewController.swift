...
final class SettingsViewController: FormViewController, Coordinator {
    ...
    override func viewDidLoad() {
        ...
        form = Section()
            ...
            /*        <<< AppFormAppearance.button { button in
             button.title = R.string.localizable.settingsHelpCenterTitle()
             }.onCellSelection { [weak self] _, _  in
             self?.showAlertController()
             }.cellUpdate({ (cell, _) in
             cell.textLabel?.textColor = .black
             //                    cell.imageView?.image = R.image.settings_colorful_about()
             cell.imageView?.image = R.image.settings_colorful_update()
             cell.textLabel?.text = R.string.localizable.settingsHelpCenterTitle()
             cell.accessoryType = .disclosureIndicator
             cell.textLabel?.textAlignment = .left
             }).cellSetup({ (cell, row) in
             cell.detailTextLabel?.text = "1.1.100"
             cell.detailTextLabel?.textColor = .black
             //                }) */
            <<< LabelRow() {
                $0.title = R.string.localizable.settingsUpdateTitle()
                $0.value = /*R.string.localizable.settingsVersionLabelTitle() + */ Bundle.main.fullVersion
                }.cellSetup({ (cell, row) in
                    cell.imageView?.image = R.image.settings_colorful_update()
                    cell.textLabel?.textColor = .black
                }).onCellSelection({ [weak self] (_, _) in
    ...
    func showAlertController() {
        var getVersion = [String: String]()
        //        let currentVersion = Bundle.main.infoDictionary!["CFBundleShortVersionString"] as! String
        let currentVersion = Bundle.main.fullVersion.replacingOccurrences(of: "(", with: "").replacingOccurrences(of: ")", with: "").replacingOccurrences(of: " ", with: ".")
        GitHubNetwork.request(RequestVersoin()) { (response) in
            getVersion = (response.data as! Dictionary)["root"]!!
            if let serverVersion = getVersion["version"] {
                switch (currentVersion.compare(serverVersion)) {
                case ComparisonResult.orderedSame:
                    let hud = MBProgressHUD.showAdded(to: self.view, animated: true)
                    hud.mode = MBProgressHUDMode.text
                    hud.label.text = R.string.localizable.version()
                    hud.hide(animated: true, afterDelay: 1.0)
                case ComparisonResult.orderedAscending:
                    
                    let alertController = UIAlertController(title: R.string.localizable.settingsUpdateTitle(),
                                                            message: R.string.localizable.newVersion(), preferredStyle: .alert)
                    let cancelAction = UIAlertAction(title: R.string.localizable.cancel(), style: .cancel, handler: nil)
                    let okAction = UIAlertAction(title: R.string.localizable.sure(), style: .default, handler: { action in
                        let url = getVersion["url"]
                        if let url = url {
                            let fullurl = "http://69.171.72.114:8088" + url
                            let urll: URL? = URL.init(string: fullurl)
                ...
                let hud = MBProgressHUD.showAdded(to: self.view, animated: true)
                hud.mode = MBProgressHUDMode.text
                hud.label.text = R.string.localizable.versionError()
                hud.hide(animated: true, afterDelay: 1.0)
            }
        }
    }
