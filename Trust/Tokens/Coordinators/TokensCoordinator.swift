...
final class TokensCoordinator: Coordinator {
    ...
    lazy var bancorExchangeViewController: BancorTokensViewController = {
        
        let controller = BancorTokensViewController(viewModel: BancorTokensViewModel(exchangeNetwork: BancorNetwork(provider: BancorProviderFactory.makeProvider())))
...
//            nonFungibleTokensViewController: nonFungibleTokensViewController
            bancorExchangeViewController: bancorExchangeViewController
        )
        ...
        //修改   增加导航左键
        masterViewController.navigationItem.leftBarButtonItem = UIBarButtonItem(image: R.image.browser_scan(), style: .done, target: self, action: #selector(openReader))
